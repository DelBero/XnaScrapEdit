...
namespace Microsoft.VisualStudio.Project
{
    public static class Utilities
    {
        ...
        /// <summary>
        /// Runs through all childnodes and compares the name
        /// </summary>
        /// <param name="node">node to check</param>
        /// <param name="name">name to check against</param>
        /// <returns>true if there is no subnode with name 'name'</returns>
        public static bool IsNameUniqueInFirstLevelOfNode(ProjectNode node, string name)
        {
            HierarchyNode child = node.FirstChild;
            while (child != null)
            {
                if (string.Compare(child.Caption, name) == 0)
                    return false;
