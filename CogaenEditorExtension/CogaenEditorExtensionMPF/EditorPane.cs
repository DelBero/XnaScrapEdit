...
namespace CogaenEditExtension
{
    ...
    //     - An empty ctl file in the same directory as NewFileItems.vsdir. In
    //       our case we chose NewTemplate.ctl. Note this file name appears in Editor.vsdir
    ...
    //          - "NewTemplate" - This is the base file name. New files will initially
    //             be named as NewTemplate1.ctl, NewTemplate2.ctl... etc.
    ///////////////////////////////////////////////////////////////////////////////
    // Editor.vsdir contents:-
    //    NewTemplate.ctl|{3085E1D6-A938-478e-BE49-3546C09A1AB1}|#106|80|#109|0|401|0|#107
    //
    // The fields in order are as follows:-
    //    - NewTemplate.ctl - our empty ctl file
                                ...
                                IVsToolboxUser      //Sends notification about Toolbox items to the owner of these items
    {
        ...
        private const string MyExtension = ".ctl";
        ...
        #region Fields
        private CogaenEditExtensionPackage myPackage;

        private CogaenEditFile openFile = null;
        ...
        //private ObjectBuilderControl objectBuilder;
        ...
        public IObjectBuilder ObjectBuilder
        {
            get
            {
                ...
                {
                    if (this.EditorControl.ObjectBuilderControl.DataContext is IObjectBuilder)
                        return this.EditorControl.ObjectBuilderControl.DataContext as IObjectBuilder;
                }
                return null;
            }
        }
        ...
        public EditorPane(CogaenEditExtensionPackage package)
            : base(null)
        {
            PrivateInit(package);
        }
        ...
        /// <summary>
        /// Initialization routine for the Editor. Loads the list of properties for the ctl document 
        /// which will show up in the properties window 
        /// </summary>
        /// <param name="package"></param>
        private void PrivateInit(CogaenEditExtensionPackage package)
        {
            myPackage = package;
            loading = false;
            ...
            //this.objectBuilder = new ObjectBuilderControl();

            resources.ApplyResources(this.editorControl, "editorControl", CultureInfo.CurrentUICulture);
            //resources.ApplyResources(this.objectBuilder, "objectBuiler", CultureInfo.CurrentUICulture);
        ...
        public bool DataChanged
        {
            get
            {
                ObjectBuilder obj = editorControl.ObjectBuilderControl.DataContext as ObjectBuilder;
                if (obj != null)
                {
                    return obj.Dirty;
                }
        ...
        private void setupCommands()
        {
            ...
            if (null != mcs)
            {
                ...
                addCommand(mcs, GuidList.guidCogaenEditExtensionCmdSet, (int)PkgCmdIDList.icmdStrike,
                ...
                // add CogaenEdit specific commands
                addCommand(mcs, GuidList.guidCogaenEditExtensionCmdSet, (int)PkgCmdIDList.cmdidConvert2Macro
                    , new EventHandler(onConvert2Macro), new EventHandler(onQueryConvert2Macro));

                addCommand(mcs, GuidList.guidCogaenEditExtensionCmdSet, (int)PkgCmdIDList.cmdidConvert2Script
                    , new EventHandler(onConvert2Script), new EventHandler(onQueryConvert2Script));

                addCommand(mcs, GuidList.guidCogaenEditExtensionCmdSet, (int)PkgCmdIDList.cmdidRunScript
                    , new EventHandler(onRunScript), new EventHandler(onQueryRunScript));
            }
        }
        ...
        /// <summary>
        /// Convert the active script to a macro
        /// </summary>
        private void onConvert2Macro(object sender, EventArgs e)
        {
            this.ObjectBuilder.IsMacro = true;
        }

        private void onQueryConvert2Macro(object sender, EventArgs e)
        {
            OleMenuCommand command = (OleMenuCommand)sender;
            if (this.ObjectBuilder != null)
                command.Enabled = !this.ObjectBuilder.IsMacro;
            else
                command.Enabled = false;
        }

        /// <summary>
        /// Convert the active macro to a script
        /// </summary>
        private void onConvert2Script(object sender, EventArgs e)
        {
            this.ObjectBuilder.IsMacro = false;
        }

        private void onQueryConvert2Script(object sender, EventArgs e)
        {
            OleMenuCommand command = (OleMenuCommand)sender;
            if (this.ObjectBuilder != null)
                command.Enabled = this.ObjectBuilder.IsMacro;
        ...
        private void onRunScript(object sender, EventArgs e)
        {
            if (this.ObjectBuilder != null)
            {
                CogaenEditExtensionPackage.MessageHandler.runScriptOnce(this.ObjectBuilder.RegisteredName, this.ObjectBuilder);
            }
        }

        private void onQueryRunScript(object sender, EventArgs e)
        {
            OleMenuCommand command = (OleMenuCommand)sender;
            command.Enabled = this.ObjectBuilder != null;
        }
        ...
        private void DisplayContextMenuAt(Point point)
        {
            ...
            CommandID contextMenuID = new CommandID(GuidList.guidCogaenEditExtensionCmdSet, PkgCmdIDList.IDMX_RTF);
        ...
        public int OpenPreloadedDocument(IObjectBuilder objectBuilder, string filename, CogaenEditFile file)
        {
            EditorControl.ObjectBuilderControl.DataContext = objectBuilder;
        ...
        int IPersistFileFormat.Load(string pszFilename, uint grfMode, int fReadOnly)
        {
            ...
            try
            {
                ...
                //try
                //{
                //    using (System.IO.Stream file = File.Open(pszFilename, FileMode.Open))
                //    {
                ...
                //catch (Exception)
                //{
                //    editorControl.ObjectBuilderControl.DataContext = null;
        ...
        {
            pfIsDirty = 0;
            IObjectBuilder obj = editorControl.ObjectBuilderControl.DataContext as IObjectBuilder;
            if (obj != null)
            {
                if (obj.Dirty)
                {
        ...
        int IPersistFileFormat.Save(string pszFilename, int fRemember, uint nFormatIndex)
        {
            ...
            if (openFile != null)
            {
                openFile.Save(pszFilename, fRemember, nFormatIndex);
            }
            else
            {
                try
                {
                    obj.serializeToXml(pszFilename);
                }
                catch (ArgumentException)
                {
        ...
        #region IDropTarget
        public void DragEnter(Microsoft.VisualStudio.OLE.Interop.IDataObject pDataObj, uint grfKeyState, POINTL pt, ref uint pdwEffect)
        {
        ...
        public void Drop(Microsoft.VisualStudio.OLE.Interop.IDataObject pDataObj, uint grfKeyState, POINTL pt, ref uint pdwEffect)
        {
            //throw new NotImplementedException();
        }
        #endregion
